...
const Productrotate:React.FC<{gender:string;imgads:string[];stopscroll:number}> = (props) => {
    ...
    let rotaerdispaly = props.imgads.map((item,index) =>
       ...
       <Singleproduct key={index} classname={`absolute ${index == 0 ? '-top-[75px] md:-top-[100px]': index == 1 ? '-left-[90px] top-[60px] md:-left-[120px] md:top-[80px]' :'top-[60px] left-[90px] md:top-[80px] md:left-[120px]'} w-full h-full border-4 border-slate-200 rounded-[50%]`} 
