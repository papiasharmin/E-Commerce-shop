...
const Allfilter :React.FC<filter>= (props) => {
  ...
  return (
    <div className='col-span-12 w-full md:col-span-3 flex flex-row md:flex-col px-2 md:px-10 py-8 gap-6 md:gap-10 text-xm md:text-base bg-slate-50'>  
        ...
        {props.product !== 'bag' && props.product !== 'sale' && props.product !== 'whatsnew' ? <Filter key={4} field={`Select size`} value={props.size ? props.size : []} name={`size`} type={'checkbox'} handelchange={props.handelsize!}/> : ''}        
