...
const AllShoes: React.FC<{allshoes:string}> = (props) => {
  const router=useRouter()
  const [product,setproduct] = useState(JSON.parse(props.allshoes))
  ...
  useEffect(()=>{
    ...
    if(catagoryfield[0] || sizefield[0]  || pricefield[0] || reviewfield){
      callapi()
       ...
       setproduct(JSON.parse(props.allshoes))
    }
...
export const getStaticProps = async() =>{
  ...
  const allshoes = await db.collection("shoe").find({gender:'male'}).project({img:1,name:1,price:1,size:1,totalrating:1,_id:0}).toArray();
  
return {
props: {allshoes:JSON.stringify(allshoes)}, // will be passed to the page component as props
}
}